#Emmett Wainwright
#HPGenerate.py
#Create a neural network and save it to an h5 file
#from a specific set of parameters

import json
import itertools
import os
os.environ['TF_CPP_MIN_LOG_LEVEL']='1' #set tenserflow logging to warning or above
...
mc = ModelCheckpoint('MNIST.h5', monitor='val_accuracy',
...
class LyrParam :
    def __init__(self, numRepsPerTest, cnnNumLayers, cnnAct, cnnChan1, cnnChan2, cnnChan3, cnnDrop,
            cnnBatch, cnnReg, denNumLayers, denNumChannels, denAct):
    ...
    def test(self):
        for x in range(self.numRepsPerTest):
            ...
            self.hst = self.nn[x].fit(datagen.flow(train_data, train_labels,
                ...
                epochs = maxNumEpochs, validation_data = (test_data, test_labels),
                callbacks=[es, mc])
...
def setupTests():
    ...
    allPermutations = [LyrParam(numReps, cnnLayers, cnnAct,
            cnnChan1, cnnChan2, cnnChan3, cnnDrop, cnnBatch, cnnReg1,
            denNumLayers, denNumChannels, denAct)
...
allConfigurations[testToGenerate-1].printParams()

#Run Test
print("\nTesting Configuration " + str(testToGenerate))
returnData = allConfigurations[testToGenerate-1].test()

#Output data to file
with open(outFilePath, "a") as outputFile:
    outputFile.write("\nConfiguration " + str(testToGenerate)
